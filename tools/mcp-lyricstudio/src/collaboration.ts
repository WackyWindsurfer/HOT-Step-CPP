...
import { MAX_POSITION_CHARS, POSITIONS_SCHEMA, DiscussionPositions } from './discussion-positions.js';
...
export class DiscussionStore {
  ...
  private agentRoster(room: string) {
    return this.presence.list(room).filter(p => p.name !== 'You');
  }
  ...
  private autoReveal(room: string, participant: string) {
    ...
    const submitted = new Set(this.positions.list(room).map(p => p.agent));
  ...
  list(limit: number) {
    return (this.db.prepare('SELECT * FROM discussions ORDER BY created_at DESC, id LIMIT ?').all(limit) as Room[]).map(r => this.normaliseRoom(r));
  }
  join(room: string, name: string, brief?: string, options: CreateOptions & { role?: string } = {}) {
    ...
    return this.db.transaction(() => {
      ...
      if (options.role?.trim()) this.db.prepare('INSERT OR REPLACE INTO discussion_roles VALUES (?, ?)').run(id, options.role.trim());
      ...
      return {
        discussion, participant_id: id, role, protocol: DISCUSSION_PROTOCOL,
  ...
  read(room: string, after: number, limit: number) {
    return this.db.transaction(() => {
      ...
      const participants = this.presence.list(room).map(p => ({ ...p, role: this.roleOf(p.id), handle: mentionHandle(p.name) }));
      return {
        discussion, messages, has_more: rows.length > limit,
        next_after_id: messages.at(-1)?.id ?? after,
        participants,
        positions: this.positions.snapshot(room, discussion.phase, participants.filter(p => p.name !== 'You')),
    ...
    return this.db.transaction(() => {
      ...
      if (options.transcript) {
        ...
        const render = (m: Message) => {
          if (m.kind === 'status' || m.kind === 'decision' || m.kind === 'agreement') {
            try {
  ...
  private checkTurn(room: string, participant: string, body?: string, readAfter?: number) {
    ...
    const last = this.db.prepare("SELECT author FROM messages WHERE room = ? AND (kind NOT IN ('status', 'coordination', 'agreement', 'position') OR author = 'You') ORDER BY id DESC LIMIT 1").get(room) as { author: string } | undefined;
  ...
  submitPosition(room: string, participant: string, request: string, body: string) {
    return this.db.transaction(() => {
      const { name } = this.participant(room, participant);
      ...
      if (previous) {
        if (previous.body !== body) throw new Error('request_id was already used for different content.');
        const current = this.room(room);
      ...
      this.positions.submit(room, agent, participant, name, body, request);
  ...
  private reveal(room: string, participant: string, request: string, reason: string) {
    const rows = this.positions.list(room);
    ...
    for (const row of rows) {
      const message = this.insert(room, row.participant_id, `position:${row.request_id}`, 'position', row.body);
...
export function registerCollaborationTools(server: McpServer, dbPath = process.env.HOTSTEP_COLLAB_DB ?? DEFAULT_COLLAB_DB) {
  ...
  server.tool('collab_join_discussion', 'Join as this chat agent. Creates a room only if missing and a brief is supplied. Returns participation instructions; read and follow them. If the room is in the sealed positions phase, submit your independent position before anything else.',
    ...
    async ({ room, name, brief, role, blind_positions, max_rounds }) => result(() => { const value = get().join(room, name, brief, { role, blind_positions, max_rounds }); joined.set(room, value.participant_id); return value; }));
  server.tool('collab_submit_position', 'Sealed positions phase only: submit your one independent position (plan, evidence, what would change your mind; max 24000 characters). Hidden from other agents until every present agent has submitted or the user reveals. Does not consume a turn.',
    { ...identity, body: z.string().trim().min(1).max(MAX_POSITION_CHARS) },
    async ({ room, participant_id, request_id, body }) => result(() => get().submitPosition(room, participant_id, request_id, body)));
