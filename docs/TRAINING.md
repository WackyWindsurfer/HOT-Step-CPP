...
- **`--attn exact|flash` (2026-09-02).** Same fused flash-attention forward/backward as the DiT (`engine/patches/flash-attn-train.patch`), with the causal mask as one triangular −INF fill — the kernel skips all-−INF tiles, so causal training gets roughly half its attention compute free. **Default `exact`; the Training Studio checkbox ships off.** What it buys, measured on this box (4B, low-VRAM path, `kinks_somethingelse` substituted for the absent nirvana fixture, paired/interleaved): against the shipped arm (`exact --attn-head-block 8`) flash is **5.5% faster per micro-step** and **3.8% lower peak VRAM**; against an equal-shape reference (`exact --attn-head-block 0`) it is only **1.2% faster** — almost all of the win is the head-block copies going away, not the fused kernel, which is the opposite split from the DiT (there the fused kernel is the win and equal-shape flash is 8.5% *slower*). Attention-only bound from `fattn-train-test --bench-lm`: 0.74×/0.79×/0.80× of the blocked arm at S=1024/2113/3500. On the naive path (0.6B/1.7B, no per-layer checkpointing) flash's linear-in-S attention state roughly **doubles the auto-fit `maxLen`** on 0.6B (3136→6208 tokens) and clears a 600 s dataset's `skippedLong` count from 2 to 0; on 1.7B the same dataset only gets a 1.27× lift and still skips one long track. 50-epoch same-seed drift (flash vs exact) is the same class as the already-shipped `--weights bf16` deviation — smaller on two of three measures, ~20% larger on final CE from a single seed, no error bar yet. Not yet ear-validated (`--attn exact --attn-head-block 8` remains the safe default until that lands). Full numbers, VRAM-model refit, and the pre-existing bugs it surfaced (an MM3 export assert, an exact-mode auto-fit OOM crash) are in `.claude/skills/flash-attn-training/SKILL.md` §3/§7.
