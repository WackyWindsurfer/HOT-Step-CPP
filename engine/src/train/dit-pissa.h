...
// The two host kernels moved to train/svd-host.h on 2026-09-05 so the LM
// trainers could share them (an LM header cannot include this file: it drags in
// the whole DiT). These forwarders keep the call sites here and in dit-hra.h
// spelled the way they always were.
static inline void dit_pissa_qr(std::vector<double> & Y, int out, int q) {
    hs_qr_mgs(Y, out, q);
}

static inline void dit_pissa_jacobi(std::vector<double> & G, int q, std::vector<double> & evals,
