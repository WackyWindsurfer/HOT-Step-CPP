...
| **LM and DiT adapters** | LoRA and LoKr on both, applied at runtime with live per-group strength dials, or merged into the weights for zero per-step cost. The DiT trainer and both LM trainers (ACE-Step and MiniMax-Music3) share DoRA, rsLoRA, LoRA+, HiRA, LoHa, PiSSA and HRA parameterizations; both LM trainers add learned artist tokens and a trainable KV prefix, stored in the same adapter file as the LoRA and applied automatically at generation on either backend. None of the extra parameterizations has beaten plain LoRA by ear yet. |
